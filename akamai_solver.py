...
class Akamai:
    ...
    A class used in order to generate Akamai Cookies

    ---------LOW + MEDIUM SECURITY ONLY---------

    for High security you need to a mouse and keyboard movements generator
...
class SizeerBot:
    def __init__(self):
        pass  # create the necessary methods and properties for this class

    def get_valid_akamai_cookie(self, client, user_agent, task_number):
        """an example of how to use the Akamai() class in order to get a valid _abck"""
        need_again = False
        while True:
            akamai_class = Akamai()
            print(
            ...
            if '~0~' in challenge_abck_cookie:
                ...
                if '||-1||' in received_abck_cookie and '~0~' in received_abck_cookie:
                    ...
                    user_agent = self.get_user_agents()
...
if __name__ == "__main__":
    CONNECTION_STRING = ""  # Need access to a valid mongodb database containing canvas information
    # generally used for browser fingerprinting
    Akamai().bpd(
        abck_cookie='4A8C24CA1D93450F0052827478673C6F~0~~-1~||1-RmFTKvpqij-1-10-1000-2||~-1'
        , has_challenge=True, is_second_challenge=True, task_number=0)

    # will need implementing remaining unresolved attributes
    bot = SizeerBot()
    args = []  # create the remaining args here
    client, valid_abck_cookie = bot.get_valid_akamai_cookie(*args)
